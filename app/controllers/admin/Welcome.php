...
class Welcome extends MY_Controller
{
    ...
    public function currentStatus(){
        $sendvalue['today_sale'] = 0;
        $sendvalue['yesterday_sale'] = 0;
        $sendvalue['current_month_sale'] = 0;
        $sendvalue['last_month_sale'] = 0;


		$this->db->select('sum(grand_total) as grand_total');
		$this->db->from('sales');
		$this->db->where('date >= "'.date("Y-m").'-01 00:00:00" and date <= "'.date('Y-m-t').' 23:59:59"');
		$query = $this->db->get();
        $d = $query->row();

		if(isset($d->grand_total)){
			$sendvalue['current_month_sale'] = decimalallow($d->grand_total,2);
		}
		else{
			$sendvalue['current_month_sale'] = 0;
		}
		...
		$this->db->select('sum(grand_total) as grand_total');
		$this->db->from('sales');
		$this->db->where('date >= "'.$lmsdate.'" and date <= "'.$lmedate.'"');
		$query = $this->db->get();
        $d = $query->row();

		if(isset($d->grand_total)){
			$sendvalue['last_month_sale'] = decimalallow($d->grand_total,2);
		}
		else{
			$sendvalue['last_month_sale'] = 0;
		}
		...
		if(isset($d->grand_total)){
			$sendvalue['today_sale'] = decimalallow($d->grand_total,2);
		}
		else{
			$sendvalue['today_sale'] = 0;
		}
        $yesterday_date = date("d")-1;
		$this->db->select('sum(grand_total) as grand_total');
		$this->db->from('sales');
		$this->db->where('date >= "'.date("Y-m").'-'. $yesterday_date.' 00:00:00" and date <= "'.date('Y-m').'-'. $yesterday_date.' 23:59:59"');
		$query = $this->db->get();
        $d = $query->row();

		if(isset($d->grand_total)){
			$sendvalue['yesterday_sale'] = decimalallow($d->grand_total,2);
		}
		else{
			$sendvalue['yesterday_sale'] = 0;
		}
